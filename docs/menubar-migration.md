...
The native AppKit accessory app has no Dock icon. An 18-point monochrome template mark uses two open brackets and a connecting dot. It exposes attached/detached/progress/error states, active foreground/background counts, connect, drain, cancel drain, explicit stop, open Desktop, status, logs and quit. The optional status window has the same actions and menu. AppKit generates all icon sizes from the same vector drawing.
