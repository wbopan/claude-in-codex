...
安装之后的全部使用说明。安装方法见 [README](README.zh-CN.md)。本文是英文版的中文翻译，内容以英文版为准。

替换已安装的版本时，先从菜单退出 App 并等待任务完成，再把新版本拖入「应用程序」。只有 ZIP 的旧版本可以解压后拖入「应用程序」安装。自己构建见[开发文档](development.md)（英文）。
