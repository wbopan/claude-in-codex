...
if (process.platform !== "darwin") throw new Error("MenuBar builds require macOS");
const publishedApp = path.join(root, ".codexhost/menubar/Codex Host.app");
...
if (processes.some((command) => command.trim().startsWith(`${publishedApp}/Contents/`)))
  throw new Error("Quit this build of Codex Host before rebuilding its bundle");
...
try {
  ...
  const executable = path.join(contents, "MacOS/CodexHost");
  execFileSync(
    "/usr/bin/xcrun",
    [
      "swiftc",
      ...
      "-framework",
      "AppKit",
      path.join(root, "apps/menubar/main.swift"),
      "-o",
      executable,
    ],
    { stdio: "inherit" },
  );
  const assets = path.join(root, ".codexhost/menubar/assets");
...
<key>CFBundleIdentifier</key><string>dev.codexhost.menubar</string>
<key>CFBundleName</key><string>Codex Host</string>
<key>CFBundleDisplayName</key><string>Codex Host</string>
<key>CFBundleExecutable</key><string>CodexHost</string>
...
<key>NSHighResolutionCapable</key><true/>
  ...
  if (latestProcesses.some((command) => command.trim().startsWith(`${publishedApp}/Contents/`)))
    throw new Error("Codex Host started while building; quit it before publishing");
