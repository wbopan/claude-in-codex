    ...
    return this.#externalRuntime.idleRelease.runOperation(childThreadId, async () => {
      try {
        ...
        const opened = await adapter.open({
          clientTools: this.#externalRuntime.clientTools(childThreadId, record.cwd),
          kind: "create",
          cwd: record.cwd,
        ...
        // Keep the native permission selected by unattended creation when this task is restored.
        // Do not reapply executionPolicy on resume: a later user permission change must win.
        const effective = thread.stateObserver.state;
        const model = input.model ?? effective.effectiveModel;
        if (model && effective.effectivePermissionModeId) {
          const transportModelId = encodeExternalTransportSelection(targetHarnessId, {
            model,
            permissionModeId: effective.effectivePermissionModeId,
            ...(effective.effectiveThinkingOptionId
              ? { thinkingOptionId: effective.effectiveThinkingOptionId }
              : {}),
          });
          record = await this.#repository.setTransportModelId(childThreadId, transportModelId);
