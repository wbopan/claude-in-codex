...
describe("HarnessDelegationCoordinator", () => {
  it.each([false, true])(
    "restores the admitted native permission, respecting later changes (%s)",
    async (userChangedMode) => {
      const full = harnessPermissionModeIdSchema.parse("bypassPermissions");
      const ask = harnessPermissionModeIdSchema.parse("default");
      ...
      try {
        const created = await value.coordinator.start({
          harnessId: "claude-code",
          task: "test",
          parentThreadId: "parent",
          cwd: "/synthetic",
        });
        const record = await value.repository.find(created.threadId);
        if (!record) throw new Error("Missing delegated task");
        const selection = decodeExternalTransportSelection("claude-code", record.transportModelId);
        expect(selection?.permissionModeId).toBe(full);
        const session = adapter.sessions[0];
        if (!session) throw new Error("Missing delegated session");
        session.succeedTurn();
        // Native resume may use its default. The Host must restore the persisted choice.
        await session.execute({ type: "permissionMode.select", permissionModeId: ask });
        if (userChangedMode) {
          await value.repository.setTransportModelId(
            record.hostThreadId,
            encodeExternalTransportSelection("claude-code", {
              ...selection,
        ...
        value.runtime.clear();
        const restored = await value.runtime.resolve(created.threadId);
        expect(restored.kind).toBe("external");
        expect(session.state.effectivePermissionModeId).toBe(userChangedMode ? ask : full);
        expect(adapter.openInputs.at(-1)?.kind).toBe("resume");
        expect(adapter.openInputs.at(-1)).not.toHaveProperty("executionPolicy");
      } finally {
        await adapter.close();
        await value.close();
      }
