...
function fixture(
  ...
  clientTools?: ClaudeSdkTransportOptions["clientTools"],
) {
...
describe("Claude SDK official Desktop MCP", () => {
  it.each(["create", "resume"] as const)(
    "registers native tools on %s with native permission handling",
    async (mode) => {
      const f = fixture(mode, "default", harnessThinkingOptionIdSchema.parse("auto"), undefined, {
        list: async () => [
          {
            namespace: "codex_app",
            definition: {
      ...
      expect(options?.mcpServers?.codex_app).toMatchObject({ type: "sdk", name: "codex_app" });
  ...
  it("does not start native work after closure during client tool discovery", async () => {
    ...
    const f = fixture("create", "default", harnessThinkingOptionIdSchema.parse("auto"), undefined, {
      list: () => discovered.promise,
      call: async () => ({ content: [] }),
      close: () => discovered.resolve([]),
    });
    const started = f.transport.start();
    const rejected = expect(started).rejects.toThrow("closing");
    await f.transport.close();
    await rejected;
